...
v0.4 の範囲は取得・抽出・実行記録・検証です。HTML レポート、分類ルール、生成 AI の助言、
他ソースとの統合は今後の対象です。
...
旧バージョンで作ったアーカイブを自動で間引くことはありません。
...
旧バージョンのアーカイブは、別途明示的に整理するまで残します。
...
### v0.1 / v0.2 / v0.3 からの更新

CLI は v0.4.0、設定スキーマは 2.1.0、state・実行記録のスキーマは 3.0.0 です。
設定 2.0.x も書き換えず読み込めます。追加した活動件数減少のしきい値は既定10％です。
Raw は DB の固定パスとなり、CSV は v0.3 の固定パスを維持します。
state 2.0.0 は検証と明示的な移行のために引き続き読み込めます。
...
v0.1 / v0.2 からは `pipeline-v1/current.json` を読み、CSV・Raw を検証して固定 CSV パスへ再抽出します。
v0.3 からは state の `current.json` を読み、既存データを検証して未変更の固定 CSV はそのまま利用します。
どちらの場合も、検証した旧 DB を最新 Raw の固定パスへコピーします。dataset ID を維持し、旧 manifest と取得来歴を新しい state の実行記録に含めます。
...
v0.2 / v0.3 の Raw は `<raw_path>/<device_id>/<run-id>/` に残せます。
新しい ingest は `<raw_path>/<device_id>/ManicTimeCore.db` と `ManicTimeReports.db` だけを更新します。
旧 DB フォルダは自動削除しないため、その容量は最新1世代分とは別に残ります。
旧データセットに対して ingest する前に `migrate-layout` を実行してください。
