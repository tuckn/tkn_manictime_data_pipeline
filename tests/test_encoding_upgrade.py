...
from manictime_pipeline.pipeline import ingest, load_current, migrate_layout, verify
...
def legacy_dataset(profile, tmp_path):
    ...
    for key, artifact in current["artifacts"].items():
        ...
        csv_path.unlink()
        artifact["path"] = f"runs/{result['run_id']}/{key}.csv"
        csv_path = profile.processed_path / "pipeline-v1" / artifact["path"]
        csv_path.parent.mkdir(parents=True, exist_ok=True)
        csv_path.write_bytes(BOM + payload)
        artifact["sha256"] = legacy_fingerprints[key]["sha256"]
        assert sha256_file(csv_path) == artifact["sha256"]
    manifest_path = profile.processed_path / "pipeline-v1/runs" / result["run_id"] / "manifest.json"
    (profile.state_path / "current.json").unlink()
    atomic_json(manifest_path, current)
    atomic_json(
        profile.processed_path / "pipeline-v1/current.json",
        {
            "schema_version": "1.0.0",
            "manifest": manifest_path.relative_to(
                profile.processed_path / "pipeline-v1"
            ).as_posix(),
...
def test_upgrade_verifies_legacy_and_publishes_bom_free_versions(profile, legacy_dataset):
    ...
    preview = migrate_layout(profile, dry_run=True)
...
def test_failed_upgrade_keeps_legacy_dataset_usable(profile, legacy_dataset, monkeypatch):
    old, preserved = legacy_dataset
    pointer = (profile.processed_path / "pipeline-v1/current.json").read_bytes()
    ...
    assert (profile.processed_path / "pipeline-v1/current.json").read_bytes() == pointer
    assert {p: sha256_file(p) for p in preserved} == preserved
    assert migrate_layout(profile, dry_run=True)["can_migrate"]
...
def test_migration_refuses_original_powershell_csv(profile, legacy_dataset):
    target = profile.processed_path / "Ar_Activity/2026/01.csv"
    target.parent.mkdir(parents=True, exist_ok=True)
    target.write_bytes(b"original PowerShell export")
    before = {p: sha256_file(p) for p in profile.processed_path.rglob("*") if p.is_file()}
    preview = migrate_layout(profile, True)
    assert not preview["can_migrate"]
    assert preview["conflicting_csvs"] == ["Ar_Activity/2026/01.csv"]
